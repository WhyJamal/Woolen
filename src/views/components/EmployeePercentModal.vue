<template>
  <div v-if="isOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-xl w-full h-full max-w-5xl mx-4 max-h-[90vh] flex flex-col">
      <div class="flex justify-between items-center px-6 rounded-md py-4 border-b bg-white">
        ...
        <div class="flex gap-6">
          <div class="flex gap-2">
            ...
            <span class="text-sm font-semibold text-blue-800">{{ props.quantity }}</span>
          </div>
            ...
            <tr v-for="(employee, index) in employees" :key="index" class="border-b border-gray-200 hover:bg-gray-50">
              <td class="px-3 py-2.5">
                <div class="flex items-center gap-1">
                  <Listbox :modelValue="employee" @update:modelValue="(val) => updateEmployee(index, val)" as="div"
                    class="flex-1">
                    <div class="relative overflow-visible">
                      ...
                      <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100"
                        ...
                        <ListboxOptions
                          ...
                          <ListboxOption v-for="emp in availableEmployees" :key="emp.GUID" :value="emp"
                            v-slot="{ active, selected }" class="cursor-pointer">
                            <div :class="[
              ...
              <td class="px-3 py-2.5">
                <input type="number" v-model.number="employee.percent"
                  class="w-full px-2 py-1.5 text-sm rounded focus:outline-none focus:ring-1 focus:ring-blue-400" min="0"
                  max="100" placeholder="0" />
              </td>
              <td class="px-3 py-2.5">
                <span class="w-full px-2 py-1.5 text-sm truncate block">{{ employee.stage.name }}</span>
              </td>
              <td class="py-2.5 px-3">
                <span class="w-full text-sm px-2 py-1.5 truncate block">{{ employee.level?.name || props.level?.name }}</span>
              </td>
              <td class="px-2 py-2.5 text-center">
                <span class="w-full text-sm text-blue-600 focus:ring-blue-500">
                  {{ employee.shift }}
                </span>
              </td>
            </tr>
      ...
      <div class="flex justify-end p-4 border-t space-x-2">
        <Button variant="secondary" @click="cancelRows">
          Аннулировать
        </Button>
...
const updateEmployee = (index, selectedEmployee) => {
  ...
  employees.value[index] = {
    ...
    stage: selectedEmployee.stage || { name: selectedEmployee.stage.name, code: selectedEmployee.stage.code },
